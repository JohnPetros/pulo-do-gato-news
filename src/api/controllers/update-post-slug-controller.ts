...
export const UpdatePostSlugController = (postsCollection: PostsCollection) => {
  return {
    async handle(http: Http<Schema>) {
      ...
      const { slug } = await http.getBody()
      const isUpdated = await postsCollection.updatePostSlug(postId, slug)
